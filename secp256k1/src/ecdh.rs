...
/// Computes the same point as [`shared_secret_point`] but with a variable-time multiply.
///
/// The timing of the multiply leaks information about `scalar`. This is intentionally accepted for
/// recipient Silent Payments scanning, where throughput matters and the scan key is local. Never use
/// it for signing or any sender path; use [`shared_secret_point`] there.
///
/// # Returns
///
/// 64 bytes representing the (x,y) co-ordinates of a point on the curve (32 bytes each).
pub fn shared_secret_point_vartime(point: &PublicKey, scalar: &SecretKey) -> [u8; 64] {
    let mut xy = [0u8; 64];

    let res = unsafe {
        ffi::secp256k1_ecdh_vartime(
            ffi::secp256k1_context_no_precomp,
            xy.as_mut_ptr(),
            point.as_c_ptr(),
            scalar.as_c_ptr(),
