...
static void bwkspscan_v0_1_0_ecmult_gen_compute_table(bwkspscan_v0_1_0_ge_storage* table, const bwkspscan_v0_1_0_ge* gen, int blocks, int teeth, int spacing) {
    size_t points = ((size_t)1) << (teeth - 1);
    size_t points_total = points * blocks;
    bwkspscan_v0_1_0_ge* prec = checked_malloc(&default_error_callback, points_total * sizeof(*prec));
    bwkspscan_v0_1_0_gej* ds = checked_malloc(&default_error_callback, teeth * sizeof(*ds));
    bwkspscan_v0_1_0_gej* vs = checked_malloc(&default_error_callback, points_total * sizeof(*vs));
    bwkspscan_v0_1_0_gej u;
    size_t vs_pos = 0;
    bwkspscan_v0_1_0_scalar half;
    int block, i;

    VERIFY_CHECK(points_total > 0);

    /* u is the running power of two times gen we're working with, initially gen/2. */
    bwkspscan_v0_1_0_scalar_half(&half, &bwkspscan_v0_1_0_scalar_one);
    bwkspscan_v0_1_0_gej_set_infinity(&u);
    for (i = 255; i >= 0; --i) {
        /* Use a very simple multiplication ladder to avoid dependency on ecmult. */
        bwkspscan_v0_1_0_gej_double_var(&u, &u, NULL);
        if (bwkspscan_v0_1_0_scalar_get_bits_limb32(&half, i, 1)) {
            bwkspscan_v0_1_0_gej_add_ge_var(&u, &u, gen, NULL);
        }
    }
#ifdef VERIFY
    {
        /* Verify that u*2 = gen. */
        bwkspscan_v0_1_0_gej double_u;
        bwkspscan_v0_1_0_gej_double_var(&double_u, &u, NULL);
        VERIFY_CHECK(bwkspscan_v0_1_0_gej_eq_ge_var(&double_u, gen));
    }
#endif
    ...
    for (block = 0; block < blocks; ++block) {
        ...
        bwkspscan_v0_1_0_gej sum;
        bwkspscan_v0_1_0_gej_set_infinity(&sum);
        for (tooth = 0; tooth < teeth; ++tooth) {
            ...
            bwkspscan_v0_1_0_gej_add_var(&sum, &sum, &u, NULL);
        ...
        bwkspscan_v0_1_0_gej_neg(&vs[vs_pos++], &sum);
        ...
        for (tooth = 0; tooth < teeth - 1; ++tooth) {
            ...
            for (index = 0; index < stride; ++index, ++vs_pos) {
                bwkspscan_v0_1_0_gej_add_var(&vs[vs_pos], &vs[vs_pos - stride], &ds[tooth], NULL);
            }
        }
    }
    VERIFY_CHECK(vs_pos == points_total);

    /* Convert all points simultaneously from bwkspscan_v0_1_0_gej to bwkspscan_v0_1_0_ge. */
    bwkspscan_v0_1_0_ge_set_all_gej_var(prec, vs, points_total);
    /* Convert all points from bwkspscan_v0_1_0_ge to bwkspscan_v0_1_0_ge_storage output. */
    for (block = 0; block < blocks; ++block) {
        size_t index;
        for (index = 0; index < points; ++index) {
            VERIFY_CHECK(!bwkspscan_v0_1_0_ge_is_infinity(&prec[block * points + index]));
            bwkspscan_v0_1_0_ge_to_storage(&table[block * points + index], &prec[block * points + index]);
        }
    }
