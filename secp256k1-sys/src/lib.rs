...
/// Callback used by silent payments light-client scanning to look up a label
/// tweak. Given the compressed 33-byte label public key and the caller's label
/// context, it returns a pointer to the label tweak if the label is in the
/// recipient's cache, or a null pointer otherwise.
pub type SilentpaymentsLabelLookup = Option<unsafe extern "C" fn(
...
extern "C" {
    ...
    // VARIABLE TIME, recipient Silent Payments scanning only, never signing.
    #[cfg_attr(not(rust_secp_no_symbol_renaming), link_name = "rustsecp256k1_v0_10_0_ecdh_vartime")]
    pub fn secp256k1_ecdh_vartime(
        cx: *const Context,
        output: *mut c_uchar,
        pubkey: *const PublicKey,
        seckey: *const c_uchar,
    ...
    // Silent payments
    #[cfg_attr(not(rust_secp_no_symbol_renaming), link_name = "rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient")]
    pub fn secp256k1_silentpayments_recipient_scan_lightclient(
        cx: *const Context,
        candidate_xonly: *mut XOnlyPublicKey,
        n_candidates: *mut size_t,
        combined_tweak: *const PublicKey,
        recipient_scan_key32: *const c_uchar,
        recipient_spend_pubkey: *const PublicKey,
        label_lookup: SilentpaymentsLabelLookup,
        label_context: *const c_void,
