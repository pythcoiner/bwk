...
int rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient_spend_points(
    const rustsecp256k1_v0_10_0_context *ctx,
    rustsecp256k1_v0_10_0_xonly_pubkey *candidate_xonly,
    size_t *n_candidates,
    const rustsecp256k1_v0_10_0_pubkey *combined_tweak,
    const unsigned char *scan_key32,
    const rustsecp256k1_v0_10_0_pubkey *spend_points,
    size_t n_spend_points
)
{
    unsigned char shared_secret33[33];
    rustsecp256k1_v0_10_0_ge combined_tweak_ge;
    rustsecp256k1_v0_10_0_scalar t0_scalar;
    rustsecp256k1_v0_10_0_gej t0_gej;
    rustsecp256k1_v0_10_0_gej candidate_gej[SP_CANDIDATE_BATCH];
    rustsecp256k1_v0_10_0_ge candidate_ge[SP_CANDIDATE_BATCH];
    size_t i;
    int ret;
    ...
    {
        rustsecp256k1_v0_10_0_scalar rsk_scalar;
        ret = rustsecp256k1_v0_10_0_scalar_set_b32_seckey(&rsk_scalar, scan_key32);
        if (!ret) {
            rustsecp256k1_v0_10_0_scalar_clear(&rsk_scalar);
            return 0;
        }
    ...
    rustsecp256k1_v0_10_0_silentpayments_create_t_k(&t0_scalar, shared_secret33, 0);
    rustsecp256k1_v0_10_0_ecmult_gen(&ctx->ecmult_gen_ctx, &t0_gej, &t0_scalar);
    ...
    for (i = 0; i < n_spend_points; i += SP_CANDIDATE_BATCH) {
        ...
        for (j = 0; j < chunk; j++) {
            rustsecp256k1_v0_10_0_ge spend_ge;
            if (!rustsecp256k1_v0_10_0_pubkey_load(ctx, &spend_ge, &spend_points[i + j])) {
                rustsecp256k1_v0_10_0_scalar_clear(&t0_scalar);
                rustsecp256k1_v0_10_0_gej_clear(&t0_gej);
                rustsecp256k1_v0_10_0_memclear(shared_secret33, sizeof(shared_secret33));
                return 0;
            }
            rustsecp256k1_v0_10_0_gej_add_ge_var(&candidate_gej[j], &t0_gej, &spend_ge, NULL);
        }
        rustsecp256k1_v0_10_0_ge_set_all_gej_var(candidate_ge, candidate_gej, chunk);
        for (j = 0; j < chunk; j++) {
            /* Adding the hashed tweak point to a valid spend point can only yield the
             * point at infinity with negligible probability. */
            if (rustsecp256k1_v0_10_0_ge_is_infinity(&candidate_ge[j])) {
                rustsecp256k1_v0_10_0_scalar_clear(&t0_scalar);
                rustsecp256k1_v0_10_0_gej_clear(&t0_gej);
                rustsecp256k1_v0_10_0_memclear(shared_secret33, sizeof(shared_secret33));
                return 0;
            }
            rustsecp256k1_v0_10_0_xonly_pubkey_save(&candidate_xonly[i + j], &candidate_ge[j]);
        }
    }
 ...
 * matches rustsecp256k1_v0_10_0_ecmult(r, a, rsk, &zero) exactly per lane, so
 * the results are byte-identical to calling ecmult once per point. */
static void rustsecp256k1_v0_10_0_silentpayments_ecdh_lockstep(
    rustsecp256k1_v0_10_0_gej *r,
    const rustsecp256k1_v0_10_0_gej *a,
    const rustsecp256k1_v0_10_0_scalar *rsk,
    size_t lanes
) {
    ...
    rustsecp256k1_v0_10_0_ge pre_a[SP_ECDH_LANES][ECMULT_TABLE_SIZE(WINDOW_A)];
    rustsecp256k1_v0_10_0_fe aux[SP_ECDH_LANES][ECMULT_TABLE_SIZE(WINDOW_A)];
    rustsecp256k1_v0_10_0_fe Z[SP_ECDH_LANES];
    rustsecp256k1_v0_10_0_ge tmpa;
    int i;
    size_t k;

    /* Split rsk once and build its shared wnaf representation. */
    rustsecp256k1_v0_10_0_scalar_split_lambda(&na_1, &na_lam, rsk);
    bits_na_1   = rustsecp256k1_v0_10_0_ecmult_wnaf(wnaf_na_1,   129, &na_1,   WINDOW_A);
    bits_na_lam = rustsecp256k1_v0_10_0_ecmult_wnaf(wnaf_na_lam, 129, &na_lam, WINDOW_A);
    ...
    for (k = 0; k < lanes; k++) {
        rustsecp256k1_v0_10_0_gej tmp = a[k];
        rustsecp256k1_v0_10_0_fe_set_int(&Z[k], 1);
        rustsecp256k1_v0_10_0_ecmult_odd_multiples_table(ECMULT_TABLE_SIZE(WINDOW_A), pre_a[k], aux[k], &Z[k], &tmp);
        rustsecp256k1_v0_10_0_ge_table_set_globalz(ECMULT_TABLE_SIZE(WINDOW_A), pre_a[k], aux[k]);
        for (i = 0; i < ECMULT_TABLE_SIZE(WINDOW_A); i++) {
            rustsecp256k1_v0_10_0_fe_mul(&aux[k][i], &pre_a[k][i].x, &rustsecp256k1_v0_10_0_const_beta);
        }
        rustsecp256k1_v0_10_0_gej_set_infinity(&r[k]);
    }
    ...
    for (i = bits - 1; i >= 0; i--) {
        int n;
        for (k = 0; k < lanes; k++) {
            rustsecp256k1_v0_10_0_gej_double_var(&r[k], &r[k], NULL);
        }
        for (k = 0; k < lanes; k++) {
            if (i < bits_na_1 && (n = wnaf_na_1[i])) {
                rustsecp256k1_v0_10_0_ecmult_table_get_ge(&tmpa, pre_a[k], n, WINDOW_A);
                rustsecp256k1_v0_10_0_gej_add_ge_var(&r[k], &r[k], &tmpa, NULL);
            }
            if (i < bits_na_lam && (n = wnaf_na_lam[i])) {
                rustsecp256k1_v0_10_0_ecmult_table_get_ge_lambda(&tmpa, pre_a[k], aux[k], n, WINDOW_A);
                rustsecp256k1_v0_10_0_gej_add_ge_var(&r[k], &r[k], &tmpa, NULL);
            }
        }
    }
    ...
    for (k = 0; k < lanes; k++) {
        if (!r[k].infinity) {
            rustsecp256k1_v0_10_0_fe_mul(&r[k].z, &r[k].z, &Z[k]);
        }
    }
}

int rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient_spend_points_batch(
    const rustsecp256k1_v0_10_0_context *ctx,
    rustsecp256k1_v0_10_0_xonly_pubkey *out_xonly,
    size_t *n_out,
    const rustsecp256k1_v0_10_0_pubkey *tweaks,
    size_t n_tweaks,
    const unsigned char *scan_key32,
    const rustsecp256k1_v0_10_0_pubkey *spend_points,
    size_t n_spend_points
)
{
    ...
    rustsecp256k1_v0_10_0_gej ss_j[SP_BATCH_TWEAK_CHUNK];
    rustsecp256k1_v0_10_0_ge ss_ge[SP_BATCH_TWEAK_CHUNK];
    rustsecp256k1_v0_10_0_scalar t0_scalar[SP_BATCH_TWEAK_CHUNK];
    rustsecp256k1_v0_10_0_gej t0_gej[SP_BATCH_TWEAK_CHUNK];
    ...
    if (!rustsecp256k1_v0_10_0_scalar_set_b32_seckey(&rsk_scalar, scan_key32)) {
        rustsecp256k1_v0_10_0_scalar_clear(&rsk_scalar);
        return 0;
    }
    rustsecp256k1_v0_10_0_declassify(ctx, &rsk_scalar, sizeof(rsk_scalar));
    ...
    for (base = 0; base < n_tweaks; base += SP_BATCH_TWEAK_CHUNK) {
        ...
        for (t = 0; t < chunk; t++) {
            rustsecp256k1_v0_10_0_ge tweak_ge;
            if (!rustsecp256k1_v0_10_0_pubkey_load(ctx, &tweak_ge, &tweaks[base + t])) {
                rustsecp256k1_v0_10_0_scalar_clear(&rsk_scalar);
                return 0;
            }
            rustsecp256k1_v0_10_0_gej_set_ge(&ss_j[t], &tweak_ge);
        }
        for (t = 0; t < chunk; t += SP_ECDH_LANES) {
            ...
            rustsecp256k1_v0_10_0_silentpayments_ecdh_lockstep(&ss_j[t], &ss_j[t], &rsk_scalar, lanes);
        }
        ...
        rustsecp256k1_v0_10_0_ge_set_all_gej_var(ss_ge, ss_j, chunk);
        for (t = 0; t < chunk; t++) {
            size_t len;
            rustsecp256k1_v0_10_0_declassify(ctx, &ss_ge[t], sizeof(ss_ge[t]));
            /* Serializing can only fail if ss is the point at infinity, which is
             * impossible for a valid tweak and scan key. */
            if (!rustsecp256k1_v0_10_0_eckey_pubkey_serialize(&ss_ge[t], shared_secret33, &len, 1)) {
                rustsecp256k1_v0_10_0_scalar_clear(&rsk_scalar);
                rustsecp256k1_v0_10_0_memclear(shared_secret33, sizeof(shared_secret33));
                return 0;
            }
            VERIFY_CHECK(len == 33);
            rustsecp256k1_v0_10_0_silentpayments_create_t_k(&t0_scalar[t], shared_secret33, 0);
        }

        /* Phase C: t_0 * G per tweak. */
        for (t = 0; t < chunk; t++) {
            rustsecp256k1_v0_10_0_ecmult_gen(&ctx->ecmult_gen_ctx, &t0_gej[t], &t0_scalar[t]);
        }
        ...
        total = chunk * n_spend_points;
        {
            ...
            for (done = 0; done < total; done += SP_CANDIDATE_BATCH) {
                ...
                for (j = 0; j < sub; j++) {
                    ...
                    if (!rustsecp256k1_v0_10_0_pubkey_load(ctx, &spend_ge, &spend_points[s])) {
                        rustsecp256k1_v0_10_0_scalar_clear(&rsk_scalar);
                        rustsecp256k1_v0_10_0_memclear(shared_secret33, sizeof(shared_secret33));
                        return 0;
                    }
                    rustsecp256k1_v0_10_0_gej_add_ge_var(&candidate_gej[j], &t0_gej[t], &spend_ge, NULL);
                }
                rustsecp256k1_v0_10_0_ge_set_all_gej_var(candidate_ge, candidate_gej, sub);
                for (j = 0; j < sub; j++) {
                    size_t flat = done + j;
                    ...
                    rustsecp256k1_v0_10_0_xonly_pubkey_save(&out_xonly[(base * n_spend_points) + flat], &candidate_ge[j]);
                }
            }
        }
    }
    ...
    for (t = 0; t < SP_BATCH_TWEAK_CHUNK; t++) {
        rustsecp256k1_v0_10_0_scalar_clear(&t0_scalar[t]);
        rustsecp256k1_v0_10_0_gej_clear(&t0_gej[t]);
    }
