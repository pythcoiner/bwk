...
int main(void) {
    ...
    ctx_size = rustsecp256k1_v0_10_0_context_preallocated_size(SECP256K1_CONTEXT_DECLASSIFY);
    ctx_memory = malloc(ctx_size);
    CHECK(ctx_memory != NULL);
    ctx = rustsecp256k1_v0_10_0_context_preallocated_create(ctx_memory, SECP256K1_CONTEXT_DECLASSIFY);
    ...
    rustsecp256k1_v0_10_0_context_preallocated_destroy(ctx);
    free(ctx_memory);
    return EXIT_SUCCESS;
}
...
static void run_tests(rustsecp256k1_v0_10_0_context *ctx, unsigned char *key) {
    ...
    key[31] ^= 1;
    CHECK(rustsecp256k1_v0_10_0_ec_pubkey_create(ctx, &recipient.labeled_spend_pubkey, key));
    key[31] ^= (1 << 1);
    recipient.index = 0;
    ...
    ret = rustsecp256k1_v0_10_0_silentpayments_recipient_create_label(ctx, &recipient.labeled_spend_pubkey, label_tweak, key, 0);
    key[31] ^= (1 << 3);
    SECP256K1_CHECKMEM_DEFINE(&ret, sizeof(ret));
    CHECK(ret == 1);
    ...
    ret = rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_create(ctx, &public_data, outpoint_smallest, xonly_pubkeys, 1, plain_pubkeys, 1);
    CHECK(ret == 1);

    tx_outputs[0] = generated_outputs[0];
    n_found_outputs = 1;
    /* Recipient scan is intentionally variable-time, so declassify the scan key for this check. */
    SECP256K1_CHECKMEM_DEFINE(&recipient.labeled_spend_pubkey, sizeof(recipient.labeled_spend_pubkey));
    SECP256K1_CHECKMEM_DEFINE(key, 32);
    ret = rustsecp256k1_v0_10_0_silentpayments_recipient_scan_outputs(ctx, found_outputs, &n_found_outputs, tx_outputs, 1, key, &public_data, &recipient.labeled_spend_pubkey, NULL, NULL);
    CHECK(ret == 1);

    /* TODO: this fails */
    /* CHECK(rustsecp256k1_v0_10_0_silentpayments_recipient_create_shared_secret(ctx, shared_secret, key, &public_data)); */
    /* TODO: test rustsecp256k1_v0_10_0_silentpayments_recipient_create_output_pubkey */
