...
fn main() {
    // Recompile the bundled C whenever any vendored source/header changes. cc only
    // tracks the explicit `.file()` entries, not the headers they `#include` (e.g.
    // the per-module `main_impl.h`), so without this an edit to an included header
    // leaves a stale object file and the link sees outdated or missing symbols.
