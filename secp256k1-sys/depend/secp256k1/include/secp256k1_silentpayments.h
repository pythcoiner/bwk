...
static const unsigned char rustsecp256k1_v0_10_0_silentpayments_public_data_magic[4] = { 0xa7, 0x1c, 0xd3, 0x5e };
 ...
 *  `rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_create`. Can be serialized as a
 *  compressed public key using
 *  `rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_serialize`. The serialization is
 *  intended for sending the public input data to light clients. Light clients
 *  can use this serialization with
 *  `rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_parse`.
 */
typedef struct {
    unsigned char data[101];
} rustsecp256k1_v0_10_0_silentpayments_recipient_public_data;
 ...
 *                     object
 */
SECP256K1_API int rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_serialize(
    const rustsecp256k1_v0_10_0_context *ctx,
 ...
 *
 *  For the labels cache, `rustsecp256k1_v0_10_0_silentpayments_recipient_create_label`
 *  can be used.
 *
 ...
 *
 *  Given the public input data (rustsecp256k1_v0_10_0_silentpayments_recipient_public_data),
...
/** Create Silent Payment candidate outputs for light client scanning.
 *
 *  Given the combined tweak `T`, a recipient's 32 byte scan key, the recipient's
 *  spend public key, an optional label lookup callback, and the label integers
 *  the caller wants to check, calculate the candidate x-only output pubkeys a
 *  light client should test against its block filter. The first candidate is the
 *  unlabeled `k = 0` output. Additional labeled variants are generated for each
 *  requested label integer that exists in the label cache.
 *
 ...
 *        recipient_scan_key32: pointer to the recipient's 32 byte scan key
 *       recipient_spend_pubkey: pointer to the recipient's spend pubkey
 *                 label_lookup: optional callback for looking up label tweaks
 *                label_context: pointer to the label context object (NULL if
 *                               labels are not used)
 *             label_integers: pointer to the label integers to check
 *         n_label_integers: the number of label integers to check
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient(
    const rustsecp256k1_v0_10_0_context *ctx,
 ...
 *
 *  Faster light-client variant of `_recipient_scan_lightclient`. Given the
 *  combined tweak `T`, the recipient's 32 byte scan key, and the recipient's
 *  precomputed spend points (the unlabeled spend pubkey first, then one
 *  `spend_pubkey + label_point` per label), derive the `k = 0` candidate x-only
 *  output key for each spend point in a single call.
 *
 *  This computes the shared secret once, derives `t_0`, computes `t_0 * G` once,
 *  and adds that point to each precomputed spend point. No labels are recomputed
 *  per call: the caller passes the spend points it already holds. The candidate
 *  order matches the spend point order.
 *
 *  The caller supplies the output buffer and `n_candidates` as the buffer
 *  capacity on input. On success, `n_candidates` is updated with the number of
 ...
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient_spend_points(
    const rustsecp256k1_v0_10_0_context *ctx,
    rustsecp256k1_v0_10_0_xonly_pubkey *candidate_xonly,
    size_t *n_candidates,
    const rustsecp256k1_v0_10_0_pubkey *combined_tweak,
 ...
 *              n_spend_points: the number of spend points
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int rustsecp256k1_v0_10_0_silentpayments_recipient_scan_lightclient_spend_points_batch(
    const rustsecp256k1_v0_10_0_context *ctx,
