...
typedef struct {
    ...
    const void* data;
} bwkspscan_v0_1_0_callback;

static SECP256K1_INLINE void bwkspscan_v0_1_0_callback_call(const bwkspscan_v0_1_0_callback * const cb, const char * const text) {
    cb->fn(text, (void*)cb->data);
}

#ifndef USE_EXTERNAL_DEFAULT_CALLBACKS
static void bwkspscan_v0_1_0_default_illegal_callback_fn(const char* str, void* data) {
    (void)data;
    fprintf(stderr, "[libsecp256k1] illegal argument: %s\n", str);
    abort();
}
static void bwkspscan_v0_1_0_default_error_callback_fn(const char* str, void* data) {
    (void)data;
    fprintf(stderr, "[libsecp256k1] internal consistency check failed: %s\n", str);
    abort();
}
#else
void bwkspscan_v0_1_0_default_illegal_callback_fn(const char* str, void* data);
void bwkspscan_v0_1_0_default_error_callback_fn(const char* str, void* data);
#endif

static const bwkspscan_v0_1_0_callback default_illegal_callback = {
    bwkspscan_v0_1_0_default_illegal_callback_fn,
    NULL
};

static const bwkspscan_v0_1_0_callback default_error_callback = {
    bwkspscan_v0_1_0_default_error_callback_fn,
    NULL
};
...
static SECP256K1_INLINE void *checked_malloc(const bwkspscan_v0_1_0_callback* cb, size_t size) {
    (void) cb;
    (void) size;
    return NULL;
}
...
static SECP256K1_INLINE void bwkspscan_v0_1_0_memczero(void *s, size_t len, int flag) {
...
static SECP256K1_INLINE void bwkspscan_v0_1_0_memclear(void *ptr, size_t len) {
#if defined(_MSC_VER)
 ...
 */
static SECP256K1_INLINE int bwkspscan_v0_1_0_memcmp_var(const void *s1, const void *s2, size_t n) {
 ...
 * Constant-time. */
static SECP256K1_INLINE int bwkspscan_v0_1_0_is_zero_array(const unsigned char *s, size_t len) {
    unsigned char acc = 0;
    int ret;
    size_t i;
    ...
    bwkspscan_v0_1_0_memclear(&acc, sizeof(acc));
    return ret;
}

/** If flag is true, set *r equal to *a; otherwise leave it. Constant-time.  Both *r and *a must be initialized and non-negative.*/
static SECP256K1_INLINE void bwkspscan_v0_1_0_int_cmov(int *r, const int *a, int flag) {
 ...
 * bwkspscan_v0_1_0_ctz32_var, but permits it to be tested separately. */
static SECP256K1_INLINE int bwkspscan_v0_1_0_ctz32_var_debruijn(uint32_t x) {
 ...
 * This function is only intended to be used as fallback for
 * bwkspscan_v0_1_0_ctz64_var, but permits it to be tested separately. */
static SECP256K1_INLINE int bwkspscan_v0_1_0_ctz64_var_debruijn(uint64_t x) {
...
/* Determine the number of trailing zero bits in a (non-zero) 32-bit x. */
static SECP256K1_INLINE int bwkspscan_v0_1_0_ctz32_var(uint32_t x) {
    VERIFY_CHECK(x != 0);
    ...
    return bwkspscan_v0_1_0_ctz32_var_debruijn(x);
#endif
}

/* Determine the number of trailing zero bits in a (non-zero) 64-bit x. */
static SECP256K1_INLINE int bwkspscan_v0_1_0_ctz64_var(uint64_t x) {
    VERIFY_CHECK(x != 0);
#if (__has_builtin(__builtin_ctzl) || SECP256K1_GNUC_PREREQ(3,4))
...
#else
    /* If no suitable CTZ builtin is available, use a (variable time) software emulation. */
    return bwkspscan_v0_1_0_ctz64_var_debruijn(x);
#endif
}

/* Read a uint32_t in big endian */
SECP256K1_INLINE static uint32_t bwkspscan_v0_1_0_read_be32(const unsigned char* p) {
...
SECP256K1_INLINE static void bwkspscan_v0_1_0_write_be32(unsigned char* p, uint32_t x) {
    p[3] = x;
    p[2] = x >>  8;
    p[1] = x >> 16;
    p[0] = x >> 24;
...
SECP256K1_INLINE static uint64_t bwkspscan_v0_1_0_read_be64(const unsigned char* p) {
...
SECP256K1_INLINE static void bwkspscan_v0_1_0_write_be64(unsigned char* p, uint64_t x) {
    p[7] = x;
    p[6] = x >>  8;
    p[5] = x >> 16;
    p[4] = x >> 24;
...
SECP256K1_INLINE static uint32_t bwkspscan_v0_1_0_rotr32(const uint32_t x, const unsigned int by) {
#if defined(_MSC_VER)
