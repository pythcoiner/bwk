...
use bwk_hwi::{ledger::LedgerSimulator, AddressScript, DeviceKind, HWI};
...
fn connect_to(sim: &Speculos) -> LedgerSimulator {
    LedgerSimulator::try_connect_on(sim.apdu_addr()).expect("connect to Speculos APDU port")
}

fn get_xpub_for_path(device: &LedgerSimulator, path: &str) -> String {
...
fn speculos_device_kind() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    assert_eq!(device.device_kind(), DeviceKind::LedgerSimulator);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_get_version() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let version = device.get_version().expect("get_version");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_master_fingerprint() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let fp = device
        .get_master_fingerprint()
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_master_fingerprint_with_custom_seed() {
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_get_extended_pubkey_bip84_testnet() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_get_extended_pubkey_bip86_testnet() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_is_wallet_registered_without_options() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/84h/1h/0h");
...
fn register_for_hmac(
    ...
    let task = std::thread::spawn(move || device.register_wallet(&name, &policy));
...
fn run_register_wsh_sortedmulti_1of2(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);

    let device_key = get_xpub_for_path(&device, "m/48h/1h/0h/2h");
    let cosigner_a = common::cosigner_origin("12345678", "m/48h/1h/0h/2h");
    let policy = format!("wsh(sortedmulti(1,{device_key}/**,{cosigner_a}/**))");

    let task = std::thread::spawn(move || device.register_wallet("test-wallet", &policy));
    ...
    match (result, ack) {
        ...
        (Err(bwk_hwi::Error::UserRefused), false) => {}
        (other, ack) => {
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_sortedmulti_1of2() {
    run_register_wsh_sortedmulti_1of2(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_sortedmulti_1of2_reject() {
    run_register_wsh_sortedmulti_1of2(false);
}

fn run_register_wsh_sortedmulti_2of3(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    ...
    let task = std::thread::spawn(move || device.register_wallet("multi-2of3", &policy));
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_sortedmulti_2of3() {
    run_register_wsh_sortedmulti_2of3(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_sortedmulti_2of3_reject() {
    run_register_wsh_sortedmulti_2of3(false);
}
...
fn run_display_wsh_sortedmulti_1of2() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/0h/2h");
    let cosigner_a = common::cosigner_origin("12345678", "m/48h/1h/0h/2h");
    ...
    let hmac = register_for_hmac(&sim, device, "multi-1of2", &policy, REGISTER_STEPS_1OF2_ACK);
    ...
    let task = std::thread::spawn(move || {
        device.display_address(&AddressScript::Miniscript {
            index: 0,
            change: false,
        })
    });
    ...
    task.join().unwrap().expect("display_address");
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_wsh_sortedmulti_1of2() {
    run_display_wsh_sortedmulti_1of2();
}

fn run_display_wsh_sortedmulti_2of3() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/0h/2h");
    let cosigner_a = common::cosigner_origin("12345678", "m/48h/1h/0h/2h");
    ...
    let hmac = register_for_hmac(&sim, device, "multi-2of3", &policy, REGISTER_STEPS_2OF3_ACK);
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_wsh_sortedmulti_2of3() {
    run_display_wsh_sortedmulti_2of3();
}
...
fn run_sign_wsh_sortedmulti_1of2(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/0h/2h");
    let cosigner_a = common::cosigner_origin("12345678", "m/48h/1h/0h/2h");
    ...
    let device = LedgerSimulator::try_connect_on(sim.apdu_addr())
        ...
        .expect("with_wallet");
    let task = std::thread::spawn(move || {
        let res = device.sign_tx(&mut psbt);
        (res, psbt)
    });
    ...
    let (api_result, psbt) = task.join().unwrap();
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_sortedmulti_1of2() {
    run_sign_wsh_sortedmulti_1of2(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_sortedmulti_1of2_reject() {
    run_sign_wsh_sortedmulti_1of2(false);
}

fn run_sign_wsh_sortedmulti_2of3(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/0h/2h");
    let cosigner_a = common::cosigner_origin("12345678", "m/48h/1h/0h/2h");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_sortedmulti_2of3() {
    run_sign_wsh_sortedmulti_2of3(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_sortedmulti_2of3_reject() {
    run_sign_wsh_sortedmulti_2of3(false);
}
...
fn run_register_wsh_miniscript_inheritance(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/4h/2h");
    let recovery_key = common::cosigner_origin("cafebabe", "m/48h/1h/2h/2h");
    let policy = inheritance_policy(&device_key, &recovery_key);

    let task = std::thread::spawn(move || device.register_wallet("inherit", &policy));
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_miniscript_inheritance() {
    run_register_wsh_miniscript_inheritance(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_wsh_miniscript_inheritance_reject() {
    run_register_wsh_miniscript_inheritance(false);
}

fn run_display_wsh_miniscript_inheritance() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/4h/2h");
    let recovery_key = common::cosigner_origin("cafebabe", "m/48h/1h/2h/2h");
    ...
    task.join().unwrap().expect("display_address inheritance");
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_wsh_miniscript_inheritance() {
    run_display_wsh_miniscript_inheritance();
}

fn run_sign_wsh_miniscript_inheritance_primary(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let device_key = get_xpub_for_path(&device, "m/48h/1h/4h/2h");
    let recovery_key = common::cosigner_origin("cafebabe", "m/48h/1h/2h/2h");
    ...
    match (api_result, ack) {
        ...
        (Err(bwk_hwi::Error::UserRefused), false) => assert!(
            psbt.inputs[0].partial_sigs.is_empty(),
            "expected no partial_sigs after nack"
        ),
        (other, ack) => {
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_miniscript_inheritance_primary() {
    run_sign_wsh_miniscript_inheritance_primary(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_miniscript_inheritance_primary_reject() {
    run_sign_wsh_miniscript_inheritance_primary(false);
}

fn run_sign_wsh_miniscript_inheritance_recovery(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    // Recovery test: device sits in the recovery branch (K1).
    let primary_key = common::cosigner_origin("c658b283", "m/48h/1h/4h/2h");
    let device_key = get_xpub_for_path(&device, "m/48h/1h/2h/2h");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_miniscript_inheritance_recovery() {
    run_sign_wsh_miniscript_inheritance_recovery(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wsh_miniscript_inheritance_recovery_reject() {
    run_sign_wsh_miniscript_inheritance_recovery(false);
}
...
fn run_sign_bip86_tr(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);

    let device_key = get_xpub_for_path(&device, "m/86h/1h/0h");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_bip86_tr() {
    run_sign_bip86_tr(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_bip86_tr_reject() {
    run_sign_bip86_tr(false);
}
...
fn probe_register_tr_1leaf(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    let leaf_key = common::cosigner_origin("d4ab66f1", "m/48h/1h/8h/2h");
    let policy = tr_1leaf_policy(&internal_key, &leaf_key);

    let task = std::thread::spawn(move || device.register_wallet("tr-1leaf", &policy));
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_tr_miniscript_1leaf() {
    probe_register_tr_1leaf(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_tr_miniscript_1leaf_reject() {
    probe_register_tr_1leaf(false);
}
...
fn run_display_tr_miniscript_1leaf() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    let leaf_key = common::cosigner_origin("d4ab66f1", "m/48h/1h/8h/2h");
    ...
    task.join().unwrap().expect("display_address tr-1leaf");
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_tr_miniscript_1leaf() {
    run_display_tr_miniscript_1leaf();
}

fn run_sign_tr_miniscript_1leaf_keypath(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    let leaf_key = common::cosigner_origin("d4ab66f1", "m/48h/1h/8h/2h");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_1leaf_keypath() {
    run_sign_tr_miniscript_1leaf_keypath(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_1leaf_keypath_reject() {
    run_sign_tr_miniscript_1leaf_keypath(false);
}

fn run_sign_tr_miniscript_1leaf_scriptpath(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    // Device sits in the leaf (K1).
    let internal_key = common::cosigner_origin("c658b283", "m/48h/1h/9h/2h");
    let leaf_key = get_xpub_for_path(&device, "m/48h/1h/8h/2h");
    let policy = tr_1leaf_policy(&internal_key, &leaf_key);
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_1leaf_scriptpath() {
    run_sign_tr_miniscript_1leaf_scriptpath(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_1leaf_scriptpath_reject() {
    run_sign_tr_miniscript_1leaf_scriptpath(false);
}
...
fn run_register_tr_miniscript_2leaf(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    ...
    let task = std::thread::spawn(move || device.register_wallet("tr-2leaf", &policy));
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_tr_miniscript_2leaf() {
    run_register_tr_miniscript_2leaf(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_register_tr_miniscript_2leaf_reject() {
    run_register_tr_miniscript_2leaf(false);
}

fn run_display_tr_miniscript_2leaf() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    let leaf_a = common::cosigner_origin("d4ab66f1", "m/48h/1h/8h/2h");
    ...
    task.join().unwrap().expect("display_address tr-2leaf");
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_tr_miniscript_2leaf() {
    run_display_tr_miniscript_2leaf();
}

fn run_sign_tr_miniscript_2leaf_keypath(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    let internal_key = get_xpub_for_path(&device, "m/48h/1h/9h/2h");
    let leaf_a = common::cosigner_origin("d4ab66f1", "m/48h/1h/8h/2h");
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_2leaf_keypath() {
    run_sign_tr_miniscript_2leaf_keypath(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_2leaf_keypath_reject() {
    run_sign_tr_miniscript_2leaf_keypath(false);
}

fn run_sign_tr_miniscript_2leaf_scriptpath(ack: bool) {
    let sim = fresh_speculos();
    let device = connect_to(&sim);
    // Device sits in leaf @1 (the older(50) recovery branch).
    let internal_key = common::cosigner_origin("c658b283", "m/48h/1h/9h/2h");
    let leaf_a = get_xpub_for_path(&device, "m/48h/1h/8h/2h");
    let leaf_b = common::cosigner_origin("e2e975b7", "m/48h/1h/5h/2h");
    let policy = tr_2leaf_policy(&internal_key, &leaf_a, &leaf_b);
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_2leaf_scriptpath() {
    run_sign_tr_miniscript_2leaf_scriptpath(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_tr_miniscript_2leaf_scriptpath_reject() {
    run_sign_tr_miniscript_2leaf_scriptpath(false);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_display_address_bip86_tr() {
    let sim = fresh_speculos();
    let device = connect_to(&sim);

    let path = DerivationPath::from_str("m/86h/1h/0h/0/0").unwrap();
    let task = std::thread::spawn(move || device.display_address(&AddressScript::P2TR(path)));
    ...
    task.join().unwrap().expect("display_address P2TR");
}

fn run_sign_wpkh(ack: bool) {
    let sim = fresh_speculos();
    let device_for_key = connect_to(&sim);
    let device_key = get_xpub_for_path(&device_for_key, "m/84h/1h/0h");
    let policy = format!("wpkh({device_key}/**)");
    drop(device_for_key);
    let device = LedgerSimulator::try_connect_on(sim.apdu_addr())
        .expect("connect")
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wpkh() {
    run_sign_wpkh(true);
}

#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wpkh_reject() {
    run_sign_wpkh(false);
}
...
#[test]
#[ignore = "requires SPECULOS_ELF"]
fn speculos_sign_wpkh_unverified_inputs() {
    let sim = fresh_speculos();
    let device_for_key = connect_to(&sim);
    let device_key = get_xpub_for_path(&device_for_key, "m/84h/1h/0h");
    let policy = format!("wpkh({device_key}/**)");
    ...
    let task = std::thread::spawn(move || {
        device_for_task.sign_tx(&mut psbt_for_task)?;
        Ok::<Psbt, bwk_hwi::Error>(psbt_for_task)
    });
    ...
    psbt = task.join().unwrap().expect("sign_tx");
