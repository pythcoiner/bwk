...
static void bwkspscan_v0_1_0_ecmult_gen_context_build(bwkspscan_v0_1_0_ecmult_gen_context *ctx) {
    bwkspscan_v0_1_0_ecmult_gen_blind(ctx, NULL);
    ctx->built = 1;
}

static int bwkspscan_v0_1_0_ecmult_gen_context_is_built(const bwkspscan_v0_1_0_ecmult_gen_context* ctx) {
    return ctx->built;
}

static void bwkspscan_v0_1_0_ecmult_gen_context_clear(bwkspscan_v0_1_0_ecmult_gen_context *ctx) {
    ctx->built = 0;
    bwkspscan_v0_1_0_scalar_clear(&ctx->scalar_offset);
    bwkspscan_v0_1_0_ge_clear(&ctx->ge_offset);
    bwkspscan_v0_1_0_fe_clear(&ctx->proj_blind);
}

/* Compute the scalar (2^COMB_BITS - 1) / 2, the difference between the gn argument to
 * bwkspscan_v0_1_0_ecmult_gen, and the scalar whose encoding the table lookup bits are drawn
 * from (before applying blinding). */
static void bwkspscan_v0_1_0_ecmult_gen_scalar_diff(bwkspscan_v0_1_0_scalar* diff) {
    int i;

    /* Compute scalar -1/2. */
    bwkspscan_v0_1_0_scalar neghalf;
    bwkspscan_v0_1_0_scalar_half(&neghalf, &bwkspscan_v0_1_0_scalar_one);
    bwkspscan_v0_1_0_scalar_negate(&neghalf, &neghalf);

    /* Compute offset = 2^(COMB_BITS - 1). */
    *diff = bwkspscan_v0_1_0_scalar_one;
    for (i = 0; i < COMB_BITS - 1; ++i) {
        bwkspscan_v0_1_0_scalar_add(diff, diff, diff);
    }

    /* The result is the sum 2^(COMB_BITS - 1) + (-1/2). */
    bwkspscan_v0_1_0_scalar_add(diff, diff, &neghalf);
}

static void bwkspscan_v0_1_0_ecmult_gen(const bwkspscan_v0_1_0_ecmult_gen_context *ctx, bwkspscan_v0_1_0_gej *r, const bwkspscan_v0_1_0_scalar *gn) {
    uint32_t comb_off;
    bwkspscan_v0_1_0_ge add;
    bwkspscan_v0_1_0_fe neg;
    bwkspscan_v0_1_0_ge_storage adds;
    bwkspscan_v0_1_0_scalar d;
    ...
    bwkspscan_v0_1_0_scalar_add(&d, &ctx->scalar_offset, gn);
    /* Convert to recoded array. */
    for (i = 0; i < 8 && i < ((COMB_BITS + 31) >> 5); ++i) {
        recoded[i] = bwkspscan_v0_1_0_scalar_get_bits_limb32(&d, 32 * i, 32);
    }
    bwkspscan_v0_1_0_scalar_clear(&d);

    /* In bwkspscan_v0_1_0_ecmult_gen_prec_table we have precomputed sums of the
     ...
     * it.
     *
     * bwkspscan_v0_1_0_ecmult_gen_prec_table[b][index] stores the table(b, m) entries. Index
    ...
    while (1) {
        ...
        for (block = 0; block < COMB_BLOCKS; ++block) {
            ...
            for (tooth = 0; tooth < COMB_TEETH; ++tooth) {
                 ...
                 * instruction. */
                uint32_t bitdata = bwkspscan_v0_1_0_rotr32(recoded[bit_pos >> 5], bit_pos & 0x1f);
            ...
            for (index = 0; index < COMB_POINTS; ++index) {
                bwkspscan_v0_1_0_ge_storage_cmov(&adds, &bwkspscan_v0_1_0_ecmult_gen_prec_table[block][index], index == abs);
            }

            /* Set add=adds or add=-adds, in constant time, based on sign. */
            bwkspscan_v0_1_0_ge_from_storage(&add, &adds);
            bwkspscan_v0_1_0_fe_negate(&neg, &add.y, 1);
            bwkspscan_v0_1_0_fe_cmov(&add.y, &neg, sign);

            /* Add the looked up and conditionally negated value to r. */
            if (EXPECT(first, 0)) {
                /* If this is the first table lookup, we can skip addition. */
                bwkspscan_v0_1_0_gej_set_ge(r, &add);
                /* Give the entry a random Z coordinate to blind intermediary results. */
                bwkspscan_v0_1_0_gej_rescale(r, &ctx->proj_blind);
                first = 0;
            } else {
                bwkspscan_v0_1_0_gej_add_ge(r, r, &add);
            }
        }
    ...
    bwkspscan_v0_1_0_gej_add_ge(r, r, &ctx->ge_offset);

    /* Cleanup. */
    bwkspscan_v0_1_0_fe_clear(&neg);
    bwkspscan_v0_1_0_ge_clear(&add);
    bwkspscan_v0_1_0_memclear(&adds, sizeof(adds));
    bwkspscan_v0_1_0_memclear(&recoded, sizeof(recoded));
}

/* Setup blinding values for bwkspscan_v0_1_0_ecmult_gen. */
static void bwkspscan_v0_1_0_ecmult_gen_blind(bwkspscan_v0_1_0_ecmult_gen_context *ctx, const unsigned char *seed32) {
    bwkspscan_v0_1_0_scalar b;
    bwkspscan_v0_1_0_scalar diff;
    bwkspscan_v0_1_0_gej gb;
    bwkspscan_v0_1_0_fe f;
    unsigned char nonce32[32];
    bwkspscan_v0_1_0_rfc6979_hmac_sha256 rng;
    unsigned char keydata[64];

    /* Compute the (2^COMB_BITS - 1)/2 term once. */
    bwkspscan_v0_1_0_ecmult_gen_scalar_diff(&diff);

    if (seed32 == NULL) {
        /* When seed is NULL, reset the final point and blinding value. */
        bwkspscan_v0_1_0_ge_neg(&ctx->ge_offset, &bwkspscan_v0_1_0_ge_const_g);
        bwkspscan_v0_1_0_scalar_add(&ctx->scalar_offset, &bwkspscan_v0_1_0_scalar_one, &diff);
        ctx->proj_blind = bwkspscan_v0_1_0_fe_one;
        return;
    }
    /* The prior blinding value (if not reset) is chained forward by including it in the hash. */
    bwkspscan_v0_1_0_scalar_get_b32(keydata, &ctx->scalar_offset);
    ...
    bwkspscan_v0_1_0_rfc6979_hmac_sha256_initialize(&rng, keydata, 64);
    bwkspscan_v0_1_0_memclear(keydata, sizeof(keydata));

    /* Compute projective blinding factor (cannot be 0). */
    bwkspscan_v0_1_0_rfc6979_hmac_sha256_generate(&rng, nonce32, 32);
    bwkspscan_v0_1_0_fe_set_b32_mod(&f, nonce32);
    bwkspscan_v0_1_0_fe_cmov(&f, &bwkspscan_v0_1_0_fe_one, bwkspscan_v0_1_0_fe_normalizes_to_zero(&f));
    ctx->proj_blind = f;

    /* For a random blinding value b, set scalar_offset=diff-b, ge_offset=bG */
    bwkspscan_v0_1_0_rfc6979_hmac_sha256_generate(&rng, nonce32, 32);
    bwkspscan_v0_1_0_scalar_set_b32(&b, nonce32, NULL);
    /* The blinding value cannot be zero, as that would mean ge_offset = infinity,
     * which bwkspscan_v0_1_0_gej_add_ge cannot handle. */
    bwkspscan_v0_1_0_scalar_cmov(&b, &bwkspscan_v0_1_0_scalar_one, bwkspscan_v0_1_0_scalar_is_zero(&b));
    bwkspscan_v0_1_0_rfc6979_hmac_sha256_finalize(&rng);
    bwkspscan_v0_1_0_ecmult_gen(ctx, &gb, &b);
    bwkspscan_v0_1_0_scalar_negate(&b, &b);
    bwkspscan_v0_1_0_scalar_add(&ctx->scalar_offset, &b, &diff);
    bwkspscan_v0_1_0_ge_set_gej(&ctx->ge_offset, &gb);

    /* Clean up. */
    bwkspscan_v0_1_0_memclear(nonce32, sizeof(nonce32));
    bwkspscan_v0_1_0_scalar_clear(&b);
    bwkspscan_v0_1_0_gej_clear(&gb);
    bwkspscan_v0_1_0_fe_clear(&f);
    bwkspscan_v0_1_0_rfc6979_hmac_sha256_clear(&rng);
}
