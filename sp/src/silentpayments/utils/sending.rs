...
use crate::silentpayments::{Error, Result};
...
///
/// Since [generate_recipient_pubkeys](crate::silentpayments::sending::generate_recipient_pubkeys) calls this function internally, it is not needed for the default sending flow.
///
/// # Arguments
///
...
fn get_a_sum_secret_keys(input: &[(SecretKey, bool)]) -> Result<SecretKey> {
    ...
    for (key, is_taproot) in input {
        ...
        if *is_taproot && parity == crate::silentpayments::secp256k1::Parity::Odd {
